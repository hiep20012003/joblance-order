...
export class OrdersService {
  ...
  async submitOrderRequirements(
    ...
    // 3Kiểm tra số lượng file (chỉ check khi có file trong requirement)
    ...
    // Upload file nếu có (sau khi validate)
    ...
    // Chuẩn bị dữ liệu update requirements
    ...
    const now = new Date();
    const expectedDeliveryDate = new Date(now.getTime() + order.expectedDeliveryDays! * 24 * 60 * 60 * 1000 * order.quantity).toISOString();
